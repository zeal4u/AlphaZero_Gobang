...
class Board(object):
    ...
    def current_state(self):
        ...
        if self.states:
            ...
            square_state[2][
                self.last_move // self.width, self.last_move % self.height] = 1.0  # third feature map, last move indication
        if len(self.states) % 2 == 0:
            square_state[3][:,
            :] = 1.0  # fourth feature map, current player is player 1?  1 for player 1；0 for player 2;
    ...
    def has_a_winner(self):
        ...
        for m in moved:
            ...
            if (w in range(width - n + 1) and h in range(height - n + 1) and
                    len(set(
                        states.get(i, -1) for i in range(m, m + n * (width + 1), width + 1))) == 1):  # right diagonal
                return True, player

            if (w in range(n - 1, width) and h in range(height - n + 1) and
                    len(set(
                        states.get(i, -1) for i in range(m, m + n * (width - 1), width - 1))) == 1):  # left diagonal
                return True, player
